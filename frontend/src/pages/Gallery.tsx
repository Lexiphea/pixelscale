            ...
            {loading ? (
                ...
                <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
                    {images.map((img) => (
                        <div
                            key={img.id}
                            className="group relative break-inside-avoid cursor-pointer overflow-hidden rounded-2xl border border-white/5 bg-[#0a0a0a] transition-[border-color,box-shadow] duration-150 hover:border-primary/40 hover:shadow-[0_0_30px_rgba(16,185,129,0.1)]"
                            onClick={() => setSelectedImage(img)}
                        >
                            <img
                                src={img.url}
                                alt={`Image ${img.id}`}
                                className="w-full h-auto block transition-[filter] duration-150 group-hover:brightness-110"
                                loading="lazy"
                            />

                            {/* Overlay Info */}
                            <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-[opacity,transform] duration-150 ease-out translate-y-2 group-hover:translate-y-0">
                            ...
                            {/* Corner Accents */}
                            <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-100">
